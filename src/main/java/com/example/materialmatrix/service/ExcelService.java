                ...
                try (InputStream materialListStream = materialResource.getInputStream()) {
                    ...
                    for (int i = 4; i <= materialSheet.getLastRowNum(); i++) {
                        ...
                        if (name.equals("Seramik") || name.equals("Kompozit") || name.equals("Beton") || 
                            ...
                            String materialFileName = material.getId() + "_" + name.replace(" ", "") + ".jpg";
                            material.setFilesDirectory("src/main/resources/static/icons/Malzeme icon/" + materialFileName);
            ...
            try {
                ...
                try (InputStream techniqueListStream = techniqueResource.getInputStream()) {
                    ...
                    for (int i = 4; i <= techniqueSheet.getLastRowNum(); i++) {
                        ...
                        Cell techniqueNameCell = row.getCell(7); // H kolonu
                        Cell featureCell = row.getCell(9); // J kolonu
                        
                        logger.debug("Processing row {}: Technique cell: {}, Feature cell: {}", 
                            i + 1, techniqueNameCell != null ? techniqueNameCell.getCellType() : "null", 
                            featureCell != null ? featureCell.getCellType() : "null");
                        
                        if (techniqueNameCell != null) {
                            String techniqueName = getCellValueAsString(techniqueNameCell);
                            logger.info("Row {}: Reading technique name: '{}'", i + 1, techniqueName);
                            ...
                            if (!techniqueName.isEmpty() && !techniqueName.equals("Teknikler") && !techniqueName.equals("Teknik Kataloğu")) {
                                if (featureCell != null) {
                                    ...
                                    if (!feature.isEmpty() && !feature.equals("Özellikler")) {
                                        techniqueFeatures.computeIfAbsent(techniqueName, k -> new LinkedHashSet<>()).add(feature);
                                        logger.info("Added feature '{}' to technique '{}'", feature, techniqueName);
                                    }
                                }
                            }
                        }
                    ...
                    for (int i = 3; i <= techniqueSheet.getLastRowNum(); i++) {
                        ...
                        // Unique teknik sırasına göre ID ata
                        ...
                        String techniqueFileName = technique.getId() + "_" + name.replace(" ", "") + ".jpg";
                        technique.setFilesDirectory("src/main/resources/static/icons/Teknikler icon/" + techniqueFileName);
            ...
            try {
                ...
                try (InputStream formListStream = formResource.getInputStream()) {
                    ...
                    for (int i = 2; i <= formSheet.getLastRowNum(); i++) {
                        ...
                        // Unique form sırasına göre ID ata
